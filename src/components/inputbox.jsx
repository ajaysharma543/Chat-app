  ...
  ({ label, type = 'text', placeholder = ' ', icon, ...rest }, ref) => {
    return (
      <div className="relative w-full p-1">
        {icon && (
          <i className={`${icon} absolute right-2.5 top-5 text-white`} />
        )}
        <input
          ref={ref}
          type={type}
          placeholder={placeholder}
          className="peer w-full h-[50px] bg-transparent border-b-2  border-white outline-none text-white text-[1rem] px-[5px] pr-[35px]"
          {...rest}
        />
